...
class update extends \phpbb\console\command\command
{
	...
	protected function configure()
	{
		$this
			...
			->setDescription('Update a phpBB Extension.')
			->addArgument('extension-name', InputArgument::REQUIRED, 'Name of the extension to update (vendor/ext-name)')
		;
	}
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		{
			$output->writeln("<error>The extension '{$name}' isn't installed.</error>");

			return 1;
		}

		$output->writeln("<info>Updating phpBB Extension {$name}</info>");
